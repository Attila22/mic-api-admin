 ...
 */
public class WineryRegionHandlerInvalidate implements Handler{
        @Override
            ...
            ctx.parse(Jackson.jsonNode())
                    ...
                    .blockingMap(pair -> {
                        try (Connection connection = ctx.get(DataSource.class).getConnection()) {
                            ...
                            {
                                return WineryQtil.invalidateRowInWineryRegion(dslContext, pair.right.getRegionId(), pair.right.getIsValid()).execute();
                            }
                        }
                    })
