 ...
 */
public class VintageInvalidate implements Handler {
    @Override
        ...
        ctx.parse(Jackson.jsonNode())
                ...
                .blockingMap(pair -> {
                    try (Connection connection = ctx.get(DataSource.class).getConnection()) {
                        ...
                        {
                            return VintageQtil.invalidateVintageTag(dslContext, pair.right.getWineId(), pair.right.getVintageTag(), pair.right.getIsValid()).execute();
                        }
                    }
                })
