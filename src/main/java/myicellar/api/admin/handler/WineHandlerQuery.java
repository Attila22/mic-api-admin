 ...
 */
public class WineHandlerQuery implements Handler {
    ...
    public WineHandlerQuery(int option) {
        this._option = option;
    }
    ...
    public void handle(Context ctx) throws Exception {
        ...
        ctx.parse(Jackson.jsonNode())
                    ...
                    try (Connection connection = ctx.get(DataSource.class).getConnection()) {
                        DSLContext dslContext = CommonQtil.getDSLCtx(connection);
                        {
                            if (_option == OPTION_ONE) {
                                return WineQtil.queryOneInWine(dslContext, pair.right.getWineId()).fetch();
                            }
                ...
                .map(rec -> jsonSuccess(rec.into(Wine.class)))
