 ...
 */
public class WineryRegionHandlerCreate implements Handler{
        ...
        public void handle(Context ctx) throws Exception {
            ...
            ctx.parse(Jackson.jsonNode())
                    .map(rootNode -> {
                        ...
                        WineryRegion obj = parseRequCntnObj(obm, WineryRegion.class, rootNode); // input values by customer should go in here
                        ...
                        try (Connection connection = ctx.get(DataSource.class).getConnection()) {
                            DSLContext dslContext = CommonQtil.getDSLCtx(connection);
                            {
                                if (pair.right != null) {
                                    ...
                                    return WineryQtil.createWineryRegion(dslContext, pair.right.getIsValid(), pair.right.getRegionAltId(),
                                            pair.right.getCountryId(), pair.right.getRegionNameEng(), pair.right.getRegionNameCht(),
                                            pair.right.getRegionNameChs(), pair.right.getNote(), pair.right.getSeoUrl()).execute();
                                }
                                else
                                    return null;
                            }
